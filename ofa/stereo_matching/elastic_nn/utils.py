...
from ofa.stereo_matching.elastic_nn.modules.dynamic_op import DynamicBatchNorm2d
...
def set_running_statistics(model, data_loader, distributed=False):
	...
	with torch.no_grad():
		...
		for sample in data_loader:
			left = sample['left'].to(get_net_device(forward_model))
			right = sample['right'].to(get_net_device(forward_model))
			forward_model(left, right)
