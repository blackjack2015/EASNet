...
from ofa.stereo_matching.data_providers import StereoDataProvider

__all__ = ['RunConfig', 'StereoRunConfig', 'DistributedStereoRunConfig']
...
class StereoRunConfig(RunConfig):
	...
	def __init__(self, n_epochs=150, init_lr=0.05, lr_schedule_type='cosine', lr_schedule_param=None,
		...
		super(StereoRunConfig, self).__init__(
	...
	def data_provider(self):
		if self.__dict__.get('_data_provider', None) is None:
			if self.dataset == StereoDataProvider.name():
				DataProviderClass = StereoDataProvider
			else:
				raise NotImplementedError
